...
async function dblogin(username) {
    const rows = await connection.execute('SELECT `Password` FROM `admin` WHERE `Username` = ?',[username]);
    return rows;
}
...
async function dbregister(name,username,password,email) {
	const mysql      = require('mysql2/promise');
	const connection = await mysql.createConnection({ host: host, user: user, password: password, database: database});
    const rows = await connection.execute('INSERT INTO `admin` (`Name`,`Username`,`Password`,`Email` ) VALUES(?,?,?,?) ',[name,username,password,email]);
    return rows;
}
